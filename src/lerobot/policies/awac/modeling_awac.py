...
from lerobot.policies.awac.configuration_awac import AWACConfig, is_image_feature
...
class AWACPolicy(
    ...
    def __init__(
        ...
        self.config = config
        self.only_off_and_intervention = self.config.only_off_and_intervention
    ...
    def get_optim_params(self) -> dict:
        ...
        add 1:取消熵正则化
        """
        self.config.use_backup_entropy = False
        return optim_params
    ...
    def predict_action_chunk(self, batch: dict[str, Tensor]) -> Tensor:
        ...
        raise NotImplementedError("AWACPolicy does not support action chunking. It returns single actions!")
    ...
    def forward(
        ...
        if model == "discrete_critic" and self.config.num_discrete_actions is not None:
            ...
            return {
                "loss_discrete_critic": loss_discrete_critic,
                "loss_bc": loss_discrete_critic.clone().detach(), 
                "loss_q": loss_discrete_critic.clone().detach()
                 }
    ...
    def compute_loss_critic(
        ...
        # 复制目标Q值，匹配多Critic的维度（num_critics, batch_size）
    ...
    def compute_loss_discrete_critic(
        ...
        # In the buffer we have the full action space (continuous + discrete)
        # We need to split them before concatenating them in the critic forward
        # 从缓冲区的连续动作和离散动作中，截取离散动作部分，四舍五入取整数
        ...
        with torch.no_grad():
            ...
            # Bellman方程：目标Q = 奖励 + 折扣因子 × (1 - 终止标志) × 下一状态最佳Q值
    ...
    def compute_loss_actor(
        ...
        beta = 2
        old_q_preds = self.critic_forward(
            observations=observations,
            actions=actions[:,:DISCRETE_DIMENSION_INDEX],
            use_target=False,
            observation_features=observation_features,
        )
        min_old_q_preds = old_q_preds.min(dim=0)[0]
        adv_pi = min_old_q_preds - min_q_preds

       
        weights = F.softmax(adv_pi / beta, dim=0)
        policy_log_prob = self.actor.get_log_probs(
                observations=observations,
                actions=actions[:,:DISCRETE_DIMENSION_INDEX],
                observation_features=observation_features,
                )
   
        actor_loss = (-policy_log_prob * len(weights) * weights.detach()).mean()
...
class SACObservationEncoder(nn.Module):
    ...
    def __init__(self, config: AWACConfig, input_normalizer: nn.Module) -> None:
        super().__init__()
        self.config = config
    ...
    def get_cached_image_features(self, obs: dict[str, Tensor], normalize: bool = False) -> dict[str, Tensor]:
        ...
        """
        """
        计算并缓存图像特征（核心优化：减少编码器调用次数）
        问题解决：你之前遇到的FPS低问题，此方法通过批量编码+缓存提升效率
        Args:
            normalize: 是否归一化（forward中已归一化，外部调用需手动开启）
        Returns:
            图像特征字典：{图像键: 特征张量}
        """
        ...
        # 批量拼接所有图像（沿批次维度dim=0，避免通道数累加）,2个图像各(batch=1,3,128,128) → 拼接后(batch=2,3,128,128)
...
class Policy(nn.Module):
    ...
    def get_std(self, observations: torch.Tensor, observation_features: torch.Tensor | None = None) -> torch.Tensor:
        obs_enc = self.encoder(observations, cache=observation_features, detach=self.encoder_is_shared)
        outputs = self.network(obs_enc)
        if self.fixed_std is None:
            log_std = self.std_layer(outputs)
...
class PretrainedImageEncoder(nn.Module):
    ...
    def _load_pretrained_vision_encoder(self, config: AWACConfig):
        """Set up CNN encoder"""
...
class TanhMultivariateNormalDiag(TransformedDistribution):
    ...
    # def entropy(self, n_samples: int = 10):
    #     action = self.base_dist.sample((n_samples,))
    #     log_probs = self.base_dist.log_prob(action)
